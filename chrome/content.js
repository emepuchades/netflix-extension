(function () {
  ...
  function initPanel(panelMode) {
    if (
      document.getElementById("netflix-filter-toggle") ||
      document.getElementById("netflix-filter-panel")
    )
      return;
    ...
    const createToggleButton = () => {
      const btn = createElement(
        ...
        {
          id: "netflix-filter-toggle",
          textContent: "⚙️ Filtros",
        }
      );
    ...
    const createFilterSection = () => {
      ...
      section.appendChild(
        createDropdown("⏱️ Duración", ["<30min", "30-60min", ">60min"])
      );
      section.appendChild(
        createDropdown("🎭 Género", ["Comedia", "Drama", "Terror", "Docu"])
      );
      section.appendChild(
        createDropdown("⭐ Valoración", ["1★+", "2★+", "3★+", "4★+", "5★"])
      );
      section.appendChild(
        createDropdown("👁️ Estado de visualización", ["Visto", "No visto"])
      );
      section.appendChild(
        createDropdown("📅 Año", [
          "2024",
      ...
      section.appendChild(
        createDropdown("🆕 Nuevas temporadas pronto", ["Sí", "No"])
      );
      return section;
    };
    ...
    const createListSection = (isModal) => {
      const title = createElement(
        isModal ? "h2" : "h4",
        { marginTop: "1rem", color: "#ffffff", marginBottom: "1.5rem" },
        {
    ...
    const createPanel = () => {
      ...
      if (galleryHeader && galleryHeader.parentElement) {
        galleryHeader.parentElement.insertBefore(
          panel,
          galleryHeader.nextSibling
        );
      } else {
    ...
    function applySeenFilter() {
      ...
      const cards = document.querySelectorAll(".rowContent");
      console.log("cards", cards);
      cards.forEach((card) => {
        const shouldHide = Math.random() < 0.3;
        if (shouldHide) {
          card.remove(); // Esto elimina completamente el nodo y reorganiza las columnas
        }
      });
    }
